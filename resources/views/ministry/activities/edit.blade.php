...
@section('title', 'Edit Activity - ' . $ministry->name)
@section('content')
<div class="space-y-4">
            ...
            <div class="flex justify-between items-center relative z-10">
                <div>
                    <div class="flex items-center mb-2">
                        <a href="{{ route('ministry.activities.show', $activity) }}" class="text-white/80 hover:text-white transition-colors mr-3">
                    ...
                    <p class="text-white/80 text-sm">Update activity details for {{ $ministry->name }}</p>
                </div>
    ...
    <div class="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
        <div class="p-6">
            <form action="{{ route('ministry.activities.update', $activity) }}" method="POST" class="space-y-6" id="activityForm">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input 
                                name="end_at" 
                                type="datetime-local" 
                                value="{{ old('end_at', $activity->end_at ? $activity->end_at->format('Y-m-d\TH:i') : '') }}" 
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#0d5c2f] focus:border-[#0d5c2f] transition-colors duration-200"
                                onchange="checkConflicts()"
                            />
                            @error('end_at')
                        ...
                        <label class="flex items-center cursor-pointer">
                            <div class="relative">
                                <input 
                                    ...
                                    value="1" 
                                    class="sr-only" 
                                    {{ old('is_all_day', $activity->is_all_day) ? 'checked' : '' }}
                                    onchange="checkConflicts()"
                                />
                        ...
                        <label class="flex items-center cursor-pointer">
                            <div class="relative">
                                <input 
                                    ...
                                    value="1" 
                                    class="sr-only" 
                                    {{ old('is_public', $activity->is_public) ? 'checked' : '' }}
                                />
                                <div class="w-10 h-6 bg-gray-300 rounded-full shadow-inner"></div>
                                <div class="dot absolute w-4 h-4 bg-white rounded-full shadow -top-1 -left-1 transition-transform duration-200 ease-in-out"></div>
                            </div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            <i class="fas fa-bullseye mr-2 text-gray-400"></i>Budget Purpose
                        </label>
                        <input 
                            name="budget_purpose" 
                            type="text" 
                            value="{{ old('budget_purpose', $activity->pendingBudgetRequest?->purpose ?? $activity->approvedBudgetRequest?->purpose ?? '') }}" 
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            <i class="fas fa-list-ul mr-2 text-gray-400"></i>Budget Breakdown
                        </label>
                        

                        <div id="budgetBreakdownList" class="space-y-3">
                            @if(isset($activity->budget_breakdown) && $activity->budget_breakdown)
                                @php
                                    $budgetItems = [];
                                    ...
                                    // If parsing failed or no items found, create a single item from estimated budget
                                    if (empty($budgetItems) && $activity->estimated_budget > 0) {
                                        $budgetItems[] = [
                                            'name' => 'General Budget',
                                            'amount' => $activity->estimated_budget_numeric
                                        ];
                                    }
                        ...
                        <div class="mt-3 p-3 bg-blue-50 rounded-lg border border-blue-200">
                            <div class="flex justify-between items-center">
                                ...
                                <span id="totalBudget" class="text-lg font-bold text-blue-900">₱{{ number_format($activity->estimated_budget_numeric ?? 0, 2) }}</span>
                            </div>
                        </div>
                        @error('budget_items')
                            <p class="text-red-600 text-sm mt-1 flex items-center">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Set initial budget item index
    const existingItems = document.querySelectorAll('.budget-item');
    budgetItemIndex = existingItems.length;
});
...
function checkConflicts() {
    ...
    let conflictData = {
        ...
        location: location,
        is_all_day: isAllDay,
        exclude_activity_id: {{ $activity->id }} // Exclude current activity from conflict check
    };
            ...
            if (data.has_conflicts) {
                ...
                if (data.conflicts.ministry_activities && data.conflicts.ministry_activities.length > 0) {
                    ...
                    data.conflicts.ministry_activities.forEach(activity => {
                        ...
                        const endTime = activity.end_at ? new Date(activity.end_at) : new Date(startTime.getTime() + 2 * 60 * 60 * 1000);
