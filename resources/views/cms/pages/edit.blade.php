...
<div class="space-y-4">
            ...
            <div class="flex justify-between items-center relative z-10">
                <div>
                    ...
                    <p class="text-white/80 mt-1 flex items-center text-sm">
                        <i class="fas fa-edit mr-2"></i>Update your page with flexible content blocks
                    </p>
                </div>
    ...
    <div class="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
        <form action="{{ isset($isStaff) && $isStaff ? route('staff.cms.pages.update', $page) : route('admin.cms.pages.update', $page) }}" method="POST" id="pageForm">
            @csrf
            @method('PUT')
    ...
    <div class="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden">
        ...
        <div class="p-4">
            <div class="bg-gray-50 rounded-lg border border-gray-200 p-4 min-h-[400px]">
                <h1 id="previewTitle" class="text-2xl font-bold text-gray-900 mb-4">{{ $page->title }}</h1>
...
@if($page->content_blocks)
    blocks = @json($page->content_blocks);
    blockCounter = blocks.length;
    // Load blocks into the interface
...
function loadExistingBlocks() {
    ...
    blocks.forEach(block => {
        ...
        // Populate the block with existing data
        populateBlockData(block);
    });
    
    updateLivePreview();
}

function populateBlockData(block) {
    ...
    switch (block.type) {
        case 'text':
            ...
            if (textArea && block.data.content) {
        ...
        case 'image':
            if (block.data.image_url) {
                updateImagePreview(block.id, block.data.image_url);
                const captionInput = blockElement.querySelector('input[type="text"]');
                if (captionInput && block.data.caption) {
        ...
        case 'gallery':
            if (block.data.images) {
                updateGalleryPreview(block.id);
            }
            break;
            
        case 'columns':
            const columnsSelect = blockElement.querySelector('select');
            if (columnsSelect && block.data.columns) {
                columnsSelect.value = block.data.columns;
            }
            
            for (let i = 1; i <= 4; i++) {
                const textArea = blockElement.querySelector(`textarea[onchange*="column${i}"]`);
                if (textArea && block.data[`column${i}`]) {
                    textArea.value = block.data[`column${i}`];
                }
            }
            break;
        ...
        case 'image_text':
            ...
            const layoutSelect = blockElement.querySelector('select[onchange*="layout"]');
            if (layoutSelect && block.data.layout) {
                layoutSelect.value = block.data.layout;
            }
            
            const contentTextArea = blockElement.querySelector('textarea[onchange*="content"]');
            if (contentTextArea && block.data.content) {
                contentTextArea.value = block.data.content;
            }
            break;
            
        case 'divider':
            const styleSelect = blockElement.querySelector('select[onchange*="style"]');
            if (styleSelect && block.data.style) {
                styleSelect.value = block.data.style;
            }
            
            const textInput = blockElement.querySelector('input[type="text"]');
            if (textInput && block.data.text) {
                textInput.value = block.data.text;
            }
            break;
            
        case 'spacer':
            const heightRange = blockElement.querySelector('input[type="range"]');
            const heightDisplay = blockElement.querySelector(`#height_${block.id}`);
            if (heightRange && block.data.height) {
                heightRange.value = block.data.height;
                if (heightDisplay) {
                    heightDisplay.textContent = block.data.height;
                }
            }
            break;
            
        case 'button':
            const buttonTextInput = blockElement.querySelector('input[type="text"]');
            if (buttonTextInput && block.data.text) {
                buttonTextInput.value = block.data.text;
            }
            
            const buttonUrlInput = blockElement.querySelector('input[type="url"]');
            if (buttonUrlInput && block.data.url) {
                buttonUrlInput.value = block.data.url;
            }
            
            const buttonStyleSelect = blockElement.querySelector('select[onchange*="style"]');
            if (buttonStyleSelect && block.data.style) {
                buttonStyleSelect.value = block.data.style;
            }
            break;
    }
}
...
function renderBlockPreview(block) {
    switch (block.type) {
        ...
        case 'spacer':
            const height = parseInt(block.data.height) || 40;
        ...
        case 'button':
            ...
            const class = styleClasses[buttonStyle] || styleClasses['primary'];
            return `<div class="text-center mb-6"><a href="${buttonUrl}" class="inline-block px-6 py-3 rounded-lg transition-colors ${class}">${buttonText}</a></div>`;
