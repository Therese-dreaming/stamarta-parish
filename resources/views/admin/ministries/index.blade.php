        ...
        <div class="px-6 py-6 relative">
            ...
            <div class="flex justify-between items-center relative z-10">
                <div>
                    ...
                    <p class="text-white/80 mt-1 text-sm">Manage ministries, heads, and status</p>
                </div>
        ...
        <div class="p-4">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-200 animate-slideInUp" style="animation-delay: {{ $loop->index * 50 }}ms">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap">
                                    <div class="flex items-center">
                                        ...
                                        <span class="text-sm text-gray-900">{{ optional($ministry->head)->name ?: '—' }}</span>
                                    </div>
                                </td>
                                <td class="px-4 py-3 whitespace-nowrap">
                                ...
                                <td class="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        <a href="{{ route('admin.ministries.edit', $ministry) }}" class="w-7 h-7 rounded-lg bg-indigo-100 hover:bg-indigo-200 flex items-center justify-center text-indigo-600 hover:text-indigo-800 transition-all duration-200 hover:scale-110" title="Edit">
                                        ...
                                        <button type="button" onclick="openModal('delete-modal-{{ $ministry->id }}')" class="w-7 h-7 rounded-lg bg-red-100 hover:bg-red-200 flex items-center justify-center text-red-600 hover:text-red-800 transition-all duration-200 hover:scale-110" title="Delete">
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden hover:shadow-lg transition-all duration-300 animate-slideInUp hover:-translate-y-1" style="animation-delay: {{ $loop->index * 100 }}ms">
                        <div class="p-4">
                            <div class="flex items-center mb-3">
                                ...
                                <div class="ml-3">
                                    <h3 class="text-base font-medium text-gray-900">{{ $ministry->name }}</h3>
                                </div>
                            </div>
                            
                            <div class="space-y-2">
                                <div class="flex justify-between items-center">
                                    <span class="text-xs text-gray-500">Head:</span>
                                    <span class="text-xs text-gray-900">{{ optional($ministry->head)->name ?: '—' }}</span>
                                </div>
            ...
            @else
                <div class="text-center py-8 animate-fadeIn">
                    ...
                    <p class="text-gray-600 text-sm">No ministries have been created yet.</p>
                </div>
            @endif
        </div>
    </div>
...
<!-- Modals for each ministry -->
@foreach($ministries as $ministry)
    <!-- Delete Modal -->
    <x-modal 
        id="delete-modal-{{ $ministry->id }}"
        title="Delete Ministry"
        message="Are you sure you want to delete {{ $ministry->name }}? This action cannot be undone and will permanently remove all associated data."
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function showTableView() {
            if (window.innerWidth >= 768) { // md breakpoint
                ...
                // Save preference
                localStorage.setItem('ministryViewPreference', 'table');
            }
        }
        ...
        function showCardView() {
            if (window.innerWidth >= 768) { // Only allow card view on desktop
                ...
                // Save preference
                localStorage.setItem('ministryViewPreference', 'card');
            }
        }
        ...
        const savedPreference = localStorage.getItem('ministryViewPreference');
        ...
        window.addEventListener('resize', function() {
            if (window.innerWidth < 768) {
                ...
                const currentPreference = localStorage.getItem('ministryViewPreference');
