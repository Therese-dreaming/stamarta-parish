...
<div class="font-[Poppins]">
    ...
    <div class="mb-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Create Parochial Activity</h1>
                <p class="text-gray-600">Add a new parish event or activity</p>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        ...
        <form action="{{ route('admin.parochial-activities.store') }}" method="POST" class="p-6">
            @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4"
                                  class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-[#0d5c2f] focus:border-[#0d5c2f]">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="date" name="event_date" id="event_date" value="{{ old('event_date') }}" required
                               min="{{ date('Y-m-d') }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-[#0d5c2f] focus:border-[#0d5c2f]">
                        <p id="event_date_help" class="mt-1 text-sm text-gray-500">Select any date that falls on the desired day of the week</p>
                        @error('event_date')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="mt-8 pt-6 border-t border-gray-200">
                ...
                <textarea name="notes" id="notes" rows="3"
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-[#0d5c2f] focus:border-[#0d5c2f]">{{ old('notes') }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
