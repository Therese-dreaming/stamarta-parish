    ...
    <div class="bg-gradient-to-r from-[#0d5c2f] to-[#0d5c2f]/90 rounded-lg shadow-md overflow-hidden">
        <div class="px-5 py-5 relative">
            ...
            <div class="flex justify-between items-center relative z-10">
                <div>
                    <h1 class="text-xl font-bold text-white">Edit FAQ</h1>
                    <p class="text-white/80 mt-1.5 text-sm flex items-center">
                        <i class="fas fa-edit mr-2 text-sm"></i>Update frequently asked question
                    </p>
                </div>
    ...
    <form action="{{ route('admin.faqs.update', $faq) }}" method="POST" class="space-y-5">
            ...
            <div class="p-5 space-y-4">
                ...
                <div class="group">
                    ...
                    <textarea name="answer" id="answer" rows="4" required
                              ...
                              placeholder="Provide a clear and helpful answer">{{ old('answer', $faq->answer) }}</textarea>
                    @error('answer')
                        <p class="mt-1 text-xs text-red-600 flex items-center">
                            <i class="fas fa-exclamation-circle mr-1 text-xs"></i>{{ $message }}
                        </p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="group">
                        ...
                        <select name="category" id="category" required
                            ...
                            <option value="services" {{ old('category', $faq->category) == 'services' ? 'selected' : '' }}>Services</option>
                            ...
                            <option value="schedule" {{ old('category', $faq->category) == 'schedule' ? 'selected' : '' }}>Schedule</option>
                            @foreach($categories as $category)
                                @if(!in_array($category, ['booking', 'services', 'general', 'payment', 'schedule']))
                                    <option value="{{ $category }}" {{ old('category', $faq->category) == $category ? 'selected' : '' }}>{{ ucfirst($category) }}</option>
                                @endif
                            @endforeach
                        </select>
                        @error('category')
                ...
                <div class="group">
                    ...
                    <input type="text" name="keywords" id="keywords" value="{{ old('keywords', $faq->formatted_keywords) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-[#0d5c2f] focus:border-[#0d5c2f] text-sm transition-all duration-200 group-hover:border-[#0d5c2f]/50"
        ...
        <div class="flex justify-end space-x-3">
            ...
            <button type="submit" 
                ...
                <i class="fas fa-save mr-1.5 text-sm group-hover:scale-110 transition-transform duration-200"></i>Update FAQ
            </button>
        </div>
    </form>
</div>
