...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Add New Priest</h1>
            <p class="text-gray-600 mt-1">Add a new priest to the parish system</p>
        </div>
        ...
        <form action="{{ route('admin.priests.store') }}" method="POST" enctype="multipart/form-data" class="p-6">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="address" name="address" rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-[#0d5c2f] focus:border-[#0d5c2f]">{{ old('address') }}</textarea>
                        @error('address')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="mt-6">
                ...
                <textarea id="bio" name="bio" rows="4"
                          class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-[#0d5c2f] focus:border-[#0d5c2f]">{{ old('bio') }}</textarea>
                @error('bio')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
